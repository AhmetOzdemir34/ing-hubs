...
export class EmployeeForm extends StoreConnectedElement {
    static properties = {
        type: {type: String},
    ...
    updated(changedProperties) {
        if (changedProperties.has('type') && this.type === 'edit') {
            const { items, editingId } = this.storeState;
            const foundItem = items.find(e => e.id === editingId);
            if (foundItem) {
                this.formData = { ...foundItem };
            }
        }
    }
    ...
    render() {
        ...
        return html`<div class="form-container">
                ...
                <div class="button-group">
                    ...
                    ${this.type === 'edit' ? 
                    html`<button @click=${this.closeEdit} class="btn btn-secondary">
                    ${lang.back}
                    </button>`: 
                    html`<button class="btn btn-secondary" @click="${this.handleCancel}">
                    ${lang.cancel}
                    </button>`}
                </div>
            </div>`;
